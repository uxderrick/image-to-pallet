...
export default function () {
  function applyColors (palette: string[]) {
    ...
    try {
      // Find the lowest point of existing frames on the page
      let lowestY = 0
      figma.currentPage.children.forEach(node => {
        if ('y' in node && 'height' in node) {
          const nodeBottom = node.y + node.height
          if (nodeBottom > lowestY) {
            lowestY = nodeBottom
          }
        }
      })
      ...
      parentFrame.name = `Color Palette ${timestamp}`
      ...
      // Position the new palette below the lowest existing frame
      parentFrame.y = lowestY + 20 // Add some padding
